...
newGrayscaleCentroid::newGrayscaleCentroid(QObject *parent)
    : QObject{parent}
{}

cv::Point2d newGrayscaleCentroid::grayCenter(const cv::Mat& TheImage, int GrayThreshold)  //二维点类型，cv::Mat&  cv::Mat 存储图像数据的矩阵类 & 按引用传递参数，避免复制整个图像数据，提高效率
    //声明一个名为grayCenter的函数，该函数接受一个只读的、按引用传递的OpenCV图像对象（cv::Mat），返回一个表示图像中某个点坐标的cv::Point对象
{
    ...
    if (totalGray > 0)
    {
        ...
        if (touchLeft || touchRight || touchTop || touchBottom)
        {
            ...
            compensationFactor = std::min(2.0, 1.0 / visibleRatio); // 限制最大补偿因子为2.0
...
cv::Point2d newGrayscaleCentroid::grayCenterLimit(const cv::Mat& TheImage, int GrayThreshold, int leftUpX, int leftUpY, int rightDownX, int rightDownY){
    {
